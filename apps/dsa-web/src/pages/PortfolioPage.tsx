...
const PortfolioPage: React.FC = () => {
  ...
  const buildPortfolioAgentContext = useCallback((): AgentHostContext => ({
    surface: 'portfolio',
    intent: 'portfolio_risk_review',
    selectedSymbols: positionRows.slice(0, 12).map((row) => row.symbol),
    portfolioScope: {
    ...
    pageState: {
      snapshot: snapshot
        ? {
            ...
            costMethod: snapshot.costMethod,
            fxStale: snapshot.fxStale,
          }
        : null,
      risk,
      topPositions: positionRows.slice(0, 8).map((row) => ({
        accountName: row.accountName,
        symbol: row.symbol,
        market: row.market,
        quantity: row.quantity,
        marketValueBase: row.marketValueBase,
        unrealizedPnlBase: row.unrealizedPnlBase,
        unrealizedPnlPct: row.unrealizedPnlPct,
        priceAvailable: hasPositionPrice(row),
      ...
      csvParseResult,
      csvCommitResult,
      safety: 'read_only_no_trade_execution',
    },
  }), [
    costMethod,
    csvCommitResult,
  ...
  const handleOpenPortfolioAgentHost = useCallback(() => {
    saveAgentHostHandoff(AGENT_HOST_PROMPTS.portfolioReview, buildPortfolioAgentContext());
    navigate(buildAgentHostChatUrl());
  }, [buildPortfolioAgentContext, navigate]);
      ...
      <section className="space-y-3">
        ...
        {hasAccounts ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_220px_420px] gap-2 items-end">
              <div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <button
                  type="button"
                  onClick={handleOpenPortfolioAgentHost}
                  disabled={!snapshot && !risk && positionRows.length === 0}
                  className="btn-secondary text-sm flex-1"
                  aria-label="带着当前持仓去问股"
                >
                  持仓诊断
                </button>
              </div>
