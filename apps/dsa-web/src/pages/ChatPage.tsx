...
import type { AgentHostContext, AgentHostHandoff } from '../utils/agentHostContext';
import {
  AGENT_HOST_HANDOFF_QUERY_KEY,
  consumeAgentHostHandoff,
...
const ChatPage: React.FC = () => {
  ...
  const [agentHostHandoff, setAgentHostHandoff] = useState<AgentHostHandoff | null>(null);
  ...
  const agentHostContextRef = useRef<AgentHostContext | null>(null);
  ...
  const handleStartNewChat = useCallback(() => {
    ...
    agentHostContextRef.current = null;
    setAgentHostHandoff(null);
  ...
  useEffect(() => {
    if (searchParams.get(AGENT_HOST_HANDOFF_QUERY_KEY) !== '1') {
      return;
    }

    const handoff = consumeAgentHostHandoff();
    if (handoff) {
      setInput(handoff.prompt);
      setAgentHostHandoff(handoff);
      agentHostContextRef.current = handoff.context;
    }
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      const context = agentHostContextRef.current
        ? { ...followUpContextRef.current, ...agentHostContextRef.current }
        : followUpContextRef.current;
      const payload = {
        message: msgText,
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {agentHostHandoff ? (
                <InlineAlert
                  variant="info"
                  title={agentHostHandoff.sourceLabel}
                  message="已带入页面上下文，发送后智能体会围绕当前页面材料继续分析。"
