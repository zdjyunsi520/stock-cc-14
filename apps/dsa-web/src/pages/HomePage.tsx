...
import {
  AGENT_HOST_PROMPTS,
  buildAgentHostChatUrl,
  compactAgentHostText,
  saveAgentHostHandoff,
  type AgentHostContext,
} from '../utils/agentHostContext';
...
const HomePage: React.FC = () => {
  ...
  const buildHomeAgentContext = useCallback((): AgentHostContext | null => {
    if (!selectedReport) {
      return null;
    }
    ...
    return {
      surface: 'home',
      intent: 'market_or_report_review',
      selectedReportId: selectedReport.meta.id !== undefined ? String(selectedReport.meta.id) : undefined,
      selectedSymbols: selectedReport.meta.stockCode ? [selectedReport.meta.stockCode] : undefined,
      pageState: {
        report: {
          id: selectedReport.meta.id,
          type: selectedReport.meta.reportType,
          stockCode: selectedReport.meta.stockCode,
          stockName: selectedReport.meta.stockName,
          createdAt: selectedReport.meta.createdAt,
          currentPrice: selectedReport.meta.currentPrice,
          changePct: selectedReport.meta.changePct,
          summary: selectedReport.summary,
          strategy: selectedReport.strategy,
        },
        marketReviewReport: compactAgentHostText(marketReviewReport),
        activeTaskCount: activeTasks.length,
        setupNeedsAction,
        setupMissingLabels,
      },
    };
  }, [activeTasks.length, marketReviewReport, selectedReport, setupMissingLabels, setupNeedsAction]);
  ...
  const handleOpenHomeAgentHost = useCallback(() => {
    const context = buildHomeAgentContext();
    if (!context) {
      return;
    }
    saveAgentHostHandoff(AGENT_HOST_PROMPTS.homeReview, context);
    navigate(buildAgentHostChatUrl());
  }, [buildHomeAgentContext, navigate]);
              ...
              <div className={isHistoryTrendOpen ? 'max-w-6xl space-y-4 pb-8' : 'max-w-4xl space-y-4 pb-8'}>
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <Button
                    ...
                    onClick={handleOpenHomeAgentHost}
                    aria-label="让智能体解读当前首页报告"
                  >
