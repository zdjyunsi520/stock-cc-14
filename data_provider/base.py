...
class BaseFetcher(ABC):
    ...
    def get_a_share_realtime_snapshot(self, stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        """获取 A 股实时快照；传入 stock_codes 时只取指定代码集合。"""
        return None
...
class DataFetcherManager:
    ...
    def get_hot_theme_universe(
        ...
        for board_type, board in ranked_boards:
            ...
            if not board_name or (board_type, board_name) in seen_boards:
                continue
            seen_boards.add((board_type, board_name))
            members = self.get_board_members(board_name, board_type, max_members_per_theme)
            for member in members:
                code = normalize_stock_code(str(member.get("code") or ""))
                name = str(member.get("name") or "")
                if not code or is_bse_code(code) or is_kc_cy_stock(code) or is_st_stock(name):
                    continue
                universe.setdefault(code, [])
                if board_name not in universe[code]:
                    universe[code].append(board_name)
        return universe
    ...
    def get_a_share_realtime_snapshot(self, stock_codes: Optional[Sequence[str]] = None) -> List[Dict[str, Any]]:
        """获取 A 股实时快照；优先小集合腾讯批量，失败后切换到 efinance/东财。"""
        ...
        for fetcher_name, method_name, source_label in routes:
            ...
            if fetcher is None:
                ...
                continue
            try:
                fetch = getattr(fetcher, method_name, None)
                if not callable(fetch):
                    last_error = f"{source_label}未实现快照接口"
                    continue
