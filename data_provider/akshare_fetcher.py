...
def _normalize_snapshot_codes(stock_codes: Optional[Sequence[str]]) -> List[str]:
    normalized: List[str] = []
    seen = set()
...
class AkshareFetcher(BaseFetcher):
    ...
    def get_a_share_realtime_snapshot(self, stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        """获取 A 股实时快照；默认按腾讯小集合、东财全量兜底。"""
        return self.get_a_share_realtime_snapshot_tencent(stock_codes) or self.get_a_share_realtime_snapshot_em(stock_codes)

    def get_a_share_realtime_snapshot_em(self, stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        """使用 akshare 东财全量接口获取 A 股快照并按代码过滤。"""
        codes = _normalize_snapshot_codes(stock_codes)
        quote = self._get_stock_realtime_quote_em(codes[0] if codes else "000001")
        if quote is not None or (_realtime_cache.get('data') is not None and not _realtime_cache.get('data').empty):
            df = _realtime_cache.get('data')
            return self._snapshot_rows_from_em_dataframe(df, codes or None)
        return None

    def get_a_share_realtime_snapshot_tencent(self, stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        """使用腾讯批量接口获取 A 股快照；传入代码时只拉取该集合。"""
        return self._get_a_share_realtime_snapshot_tencent(stock_codes)

    @staticmethod
    def _snapshot_rows_from_em_dataframe(df: Optional[pd.DataFrame], stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        if df is None or df.empty:
            return None
        wanted = set(_normalize_snapshot_codes(stock_codes)) if stock_codes else None
    ...
    def _get_a_share_realtime_snapshot_tencent(self, stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        """使用腾讯批量 quote 接口获取 A 股实时快照。"""
        try:
            from src.data.stock_index_loader import get_stock_name_index_map

            stock_map = get_stock_name_index_map()
            requested = set(_normalize_snapshot_codes(stock_codes)) if stock_codes else None
            codes: List[str] = []
            name_by_code: Dict[str, str] = {}
            seen = set()
            for key, name in stock_map.items():
                code = normalize_stock_code(str(key))
                if code in seen or len(code) != 6:
                    continue
                if requested is not None and code not in requested:
                    continue
                if is_bse_code(code) or is_kc_cy_stock(code) or is_st_stock(str(name)):
                    continue
                codes.append(code)
            ...
            if requested is not None:
                for code in requested:
                    if code in seen or len(code) != 6:
                        continue
                    if is_bse_code(code) or is_kc_cy_stock(code):
                        continue
                    codes.append(code)
                    seen.add(code)
            rows: List[Dict[str, Any]] = []
            ...
            for offset in range(0, len(codes), 120):
                ...
                symbols = ','.join(_to_sina_tx_symbol(code) for code in batch)
                response = requests.get(f"http://{TENCENT_REALTIME_ENDPOINT}={symbols}", headers=headers, timeout=8)
                response.encoding = 'gbk'
                if response.status_code != 200:
                    continue
                rows.extend(self._parse_tencent_snapshot_payload(response.text, name_by_code))
            return rows or None
        except Exception as exc:
            scope_text = f"指定 {len(stock_codes)} 只" if stock_codes else "全市场"
            logger.warning("[腾讯] 获取 A 股实时快照失败(scope=%s): %s", scope_text, exc)
            return None

    @staticmethod
    def _parse_tencent_snapshot_payload(content: str, name_by_code: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        rows: List[Dict[str, Any]] = []
        for part in content.split(';'):
            ...
            if data_start == -1 or data_end <= data_start:
                continue
            fields = part[data_start + 1:data_end].split('~')
            if len(fields) < 50:
                continue
            code = str(fields[2]).strip() if len(fields) > 2 else ""
            if not code:
                continue
            ...
            rows.append({
                'code': code,
                'name': (name_by_code or {}).get(code) or (fields[1] if len(fields) > 1 else ""),
    ...
    def get_board_members(
        ...
        try:
            ...
            if normalized_type in {"industry", "sector"}:
                logger.info(f"[API调用] ak.stock_board_industry_cons_em(symbol={name}) 获取行业成分股...")
                df = ak.stock_board_industry_cons_em(symbol=name)
                source = "akshare_industry_cons_em"
            else:
                logger.info(f"[API调用] ak.stock_board_concept_cons_em(symbol={name}) 获取概念成分股...")
            ...
            code_col = _first_existing_column(df, ("代码", "股票代码", "证券代码", "symbol", "code"))
            name_col = _first_existing_column(df, ("名称", "股票名称", "证券简称", "name"))
            change_col = _first_existing_column(df, ("涨跌幅", "涨幅", "change_pct"))
            price_col = _first_existing_column(df, ("最新价", "收盘", "price", "close"))
            if code_col is None:
                return None
            ...
            for _, row in df.iterrows():
                ...
                if change_col:
                    item["change_pct"] = safe_float(row.get(change_col))
                if price_col:
                    item["price"] = safe_float(row.get(price_col))
                rows.append(item)
                if len(rows) >= max_members:
                    break
            return rows or None
        except Exception as e:
            logger.warning(f"[Akshare] 获取板块成分股失败 board={name} type={board_type}: {e}")
            return None
