...
class EfinanceFetcher(BaseFetcher):
    ...
    def get_a_share_realtime_snapshot(self, stock_codes: Optional[Sequence[str]] = None) -> Optional[List[Dict[str, Any]]]:
        """使用 efinance 全量实时接口获取 A 股快照并按代码过滤。"""
        first_code = normalize_stock_code(str(next(iter(stock_codes or []), "000001")))
        quote = self.get_realtime_quote(first_code)
        if quote is None and (_realtime_cache.get('data') is None or _realtime_cache.get('data').empty):
            return None
        ...
        for _, row in df.iterrows():
            code = normalize_stock_code(str(row.get(code_col, "")))
