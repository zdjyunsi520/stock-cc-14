...
class FeishuStreamHandler:
    ...
    def __init__(
        ...
        self._long_task_queue: deque[tuple[str, BotMessage]] = deque()
        self._active_long_task_keys: Set[str] = set()
        self._long_task_worker_running = False
    ...
    def _long_task_key(self, bot_message: BotMessage) -> Optional[str]:
        """Return a dedupe key for long-running commands handled by the bot queue."""
        try:
            prefix = getattr(get_config(), "bot_command_prefix", "/") or "/"
            cmd_name, _ = bot_message.get_command_and_args(prefix)
        except Exception:
            cmd_name = None
        if cmd_name not in {"intraday_pick", "ipick", "盘中选股", "热点选股"}:
            return None
        scope = bot_message.chat_id or bot_message.user_id or "unknown-chat"
        return f"intraday_pick:{scope}"
    ...
    def _enqueue_message(self, bot_message: BotMessage) -> None:
        ...
        long_task_key = self._long_task_key(bot_message)
        if long_task_key:
            self._enqueue_long_task(long_task_key, bot_message)
            return
    ...
    def _enqueue_long_task(self, task_key: str, bot_message: BotMessage) -> None:
        """Queue one long-running command globally and ACK immediately."""
        should_start_worker = False
        duplicate = False
        with self._queue_lock:
            if task_key in self._active_long_task_keys:
                duplicate = True
            else:
                self._active_long_task_keys.add(task_key)
                self._long_task_queue.append((task_key, bot_message))
                if not self._long_task_worker_running:
                    self._long_task_worker_running = True
                    should_start_worker = True

        if duplicate:
            ack_ok = self._reply_client.reply_text(
                message_id=bot_message.message_id,
                text="已收到盘中选股指令；当前同一任务已在排队或执行中，本次不重复执行。完成后会在当前会话推送结果。",
                at_user=True,
                user_id=bot_message.user_id,
            )
            if not ack_ok:
                self._logger.warning("[Feishu Stream] 盘中选股重复任务 ACK 发送失败: %s", bot_message.message_id)
            return

        ack_ok = self._reply_client.reply_text(
            message_id=bot_message.message_id,
            text="已收到盘中选股指令，已进入任务队列。系统会按顺序执行，完成后会在当前会话推送结果。",
            at_user=True,
            user_id=bot_message.user_id,
        )
        if not ack_ok:
            self._logger.warning("[Feishu Stream] 盘中选股任务 ACK 发送失败: %s", bot_message.message_id)
        if should_start_worker:
            try:
                self._executor.submit(self._drain_long_tasks)
            except RuntimeError as exc:
                with self._queue_lock:
                    self._long_task_worker_running = False
                    self._active_long_task_keys.discard(task_key)
                    self._long_task_queue = deque(
                        item for item in self._long_task_queue if item[0] != task_key
                    )
                self._logger.error("[Feishu Stream] 无法启动长任务处理线程: %s", exc)
    ...
    def _drain_long_tasks(self) -> None:
        """Drain long-running bot commands in process-wide FIFO order."""
        while True:
            with self._queue_lock:
                if not self._long_task_queue:
                    self._long_task_worker_running = False
                    return
                task_key, bot_message = self._long_task_queue.popleft()

            try:
                response = self._on_message(bot_message)
                if response and response.text:
                    self._send_long_task_result(bot_message, response)
            except Exception as e:
                self._logger.error("[Feishu Stream] 长任务处理失败: %s", e)
                self._logger.exception(e)
                self._send_long_task_result(
                    bot_message,
                    BotResponse.error_response(f"任务执行失败: {str(e)[:100]}"),
                )
            finally:
                with self._queue_lock:
                    self._active_long_task_keys.discard(task_key)

    def _send_long_task_result(self, bot_message: BotMessage, response: BotResponse) -> None:
        """Send long task result as a new chat message when possible."""
        if bot_message.chat_id:
            try:
                sent = self._reply_client.send_to_chat(bot_message.chat_id, response.text)
            except Exception as exc:
                sent = False
                self._logger.error("[Feishu Stream] 长任务结果主动推送异常: %s", exc)
            if sent:
                return
            self._logger.warning(
                "[Feishu Stream] 长任务结果主动推送失败，回退回复原消息: %s",
                bot_message.message_id,
            )
        try:
            sent = self._reply_client.reply_text(
                message_id=bot_message.message_id,
                text=response.text,
            ...
            if not sent:
                self._logger.error("[Feishu Stream] 长任务结果回复失败: %s", bot_message.message_id)
        except Exception as exc:
            self._logger.error("[Feishu Stream] 长任务结果回复异常: %s", exc)
    ...
    def shutdown(self, wait: bool = False) -> None:
        ...
        with self._queue_lock:
            ...
            self._long_task_queue.clear()
            self._active_long_task_keys.clear()
            self._long_task_worker_running = False
