# -*- coding: utf-8 -*-
"""全市场低位吸筹选股 v3 - 严格版（不限题材）。

与 v2 的差异：
  - 候选池：全 A 股（不依赖题材匹配）
  - 排除板块：创业板（300xxx）、科创板（688xxx）、北交所（8xxxxx / 920xxx）
  - 排除 ST / *ST / 退市
  - 保持 v2 的低位+趋势过滤逻辑（剔除下跌中续）

过滤条件：
  1. 价格区间：20 日累计涨幅 ∈ [-8%, +12%]
  2. 距 20 日高点回撤 <= -3%（不在高点）
  3. 距 MA20 ∈ [-5%, +10%]
  4. 距 20 日低点反弹 <= 15%（不在反弹高位）
  5. 20 日内单日最大涨幅 <= 8%（无脉冲）
  6. 20 日振幅 <= 1.35
  7. 20 日无涨停（涨幅 < 9.8%）
  8. 趋势向上：MA5 >= MA10 * 0.98
  9. 低点抬高：近 5 日最低价 >= 前 5 日最低价 * 0.98
  10. 温和量能：近 5 日均量 / 近 20 日均量 <= 1.5
  11. 规律+洗盘：score <= 50 且 wash >= 10（v3 主力吸筹定义）
"""
...
MIN_RET_20D = -8.0
MAX_RET_20D = 12.0
MIN_DRAWDOWN_FROM_HIGH = -3.0
MIN_DIST_MA20 = -5.0
MAX_DIST_MA20 = 10.0
MAX_BOUNCE_FROM_LOW = 15.0
MAX_SINGLE_DAY_GAIN = 8.0
MAX_AMPLITUDE_RATIO = 1.35
MAX_VOL_RATIO_5_20 = 1.5
MA5_MA10_TOLERANCE = 0.98
LOW_LIFT_TOLERANCE = 0.98


def is_excluded_board(code: str) -> bool:
    """排除创业板（300/301）、科创板（688/689）、北交所（8/4/920）。"""
    c = code.strip()
    if c.startswith("30"):
        return True  # 创业板
...
def load_universe() -> dict:
    """加载全 A 股代码-名称，排除创业板/科创板/北交所/ST。"""
    csv_path = ROOT / "data" / "_tmp_theme_filter" / "a_code_name.csv"
...
def main() -> int:
    pool = load_universe()
    print(f"候选池（全 A 股，排除 300/688/北交所/ST）: {len(pool)} 只")
    ...
    out_full = ROOT / "data" / "_tmp_theme_filter" / "low_position_accumulation_v3.json"
    with open(out_full, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=str)
    print(f"完整结果: {out_full}")

    print("\n" + "=" * 160)
    print(f"{'排名':<4}{'代码':<8}{'名称':<14}{'最新日':<12}{'收盘':>8}"
          f"{'20日涨':>8}{'距高':>7}{'MA20%':>7}{'振幅':>6}{'MA5':>7}{'MA10':>7}"
          f"{'近5日低':>9}{'前5日低':>9}{'量比5/20':>9}{'规律':>6}{'洗盘':>5}{'吸筹天':>7}")
    print("-" * 160)
