...
def main() -> int:
    ...
    if not bot_clients_started and (args.schedule or config.schedule_enabled):
        start_bot_stream_clients(config)
        bot_clients_started = True
    ...
    try:
        ...
        if args.schedule or config.schedule_enabled:
            ...
            if getattr(config, 'realtime_watch_enabled', False):
                from src.services.realtime_watch_worker import RealtimeWatchWorker

                interval_seconds = max(30, int(getattr(config, 'realtime_watch_interval_seconds', 60) or 60))
                realtime_watch_worker = RealtimeWatchWorker(config_provider=_reload_runtime_config)

                def realtime_watch_task():
                    stats = realtime_watch_worker.run_once()
                    signal_count = stats.get("signals", 0)
                    if signal_count:
                        logger.info("[RealtimeWatch] 本轮触发 %d 条实时盯盘信号", signal_count)

                background_tasks.append({
                    "task": realtime_watch_task,
                    "interval_seconds": interval_seconds,
                    "run_immediately": True,
                    "name": "realtime_watch",
                })
                logger.info("实时盯盘已启用，轮询间隔: %d 秒", interval_seconds)

            if getattr(config, 'intraday_pick_enabled', False):
                from src.services.intraday_pick_scheduler import IntradayPickScheduler

                interval_seconds = max(30, int(getattr(config, 'intraday_pick_interval_seconds', 60) or 60))
                intraday_picker = IntradayPickScheduler(config_provider=_reload_runtime_config)

                def intraday_pick_task():
                    stats = intraday_picker.run_once()
                    if not stats.get("skipped"):
                        logger.info(
                            "[IntradayPick] slot=%s rough=%s checked=%s ranked=%s notified=%s claude=%s",
                            stats.get("slot"),
                            stats.get("rough"),
                            stats.get("checked"),
                            stats.get("ranked"),
                            stats.get("notified"),
                            stats.get("used_claude"),
                        )

                background_tasks.append({
                    "task": intraday_pick_task,
                    "interval_seconds": interval_seconds,
                    "run_immediately": True,
                    "name": "intraday_pick",
                })
                logger.info(
                    "盘中选股自动推送已启用，轮询间隔: %d 秒，推送时间: %s",
                    interval_seconds,
                    ",".join(getattr(config, 'intraday_pick_push_times', []) or []),
                )
