...
class LLMChannelConfigTestCase(unittest.TestCase):
    ...
    @patch("src.config.setup_env")
    @patch.object(Config, "_parse_litellm_yaml", return_value=[])
    def test_anthropic_env_key_defaults_to_opus_and_enables_agent(self, _mock_parse_yaml, _mock_setup_env) -> None:
        env = {
        ...
        self.assertEqual(config.anthropic_api_keys, ["sk-ant-test-value"])
        self.assertEqual(config.litellm_model, "anthropic/claude-opus-4-6")
        self.assertEqual(config.anthropic_model, "claude-opus-4-6")
        self.assertEqual(config.agent_litellm_model, "")
        self.assertEqual(get_effective_agent_primary_model(config), "anthropic/claude-opus-4-6")
