...
class MainScheduleModeTestCase(unittest.TestCase):
    ...
    def test_schedule_mode_registers_realtime_watch_background_task(self) -> None:
        args = self._make_args(schedule=True)
        config = self._make_config(
            schedule_enabled=False,
            realtime_watch_enabled=True,
            realtime_watch_interval_seconds=30,
        )
        worker = MagicMock()
        worker.run_once.return_value = {"signals": 3}
        scheduled_call = {}
        ...
        with patch("main.parse_arguments", return_value=args), \
             ...
             patch("main.setup_logging"), \
             patch("main.run_full_analysis") as run_full_analysis, \
             patch("src.services.realtime_watch_worker.RealtimeWatchWorker", return_value=worker) as worker_cls, \
             patch("src.scheduler.run_with_schedule", side_effect=fake_run_with_schedule):
            exit_code = main.main()
        ...
        self.assertEqual(len(scheduled_call["background_tasks"]), 1)
        background_task = scheduled_call["background_tasks"][0]
        self.assertEqual(background_task["name"], "realtime_watch")
        ...
        worker.run_once.assert_called_once_with()
        info_log.assert_any_call("[RealtimeWatch] 本轮触发 %d 条实时盯盘信号", 3)
