...
class TestAskCommandMultiStock(unittest.TestCase):
    ...
    def test_analyze_multi_includes_portfolio_overlay(self):
        ...
        self.assertIn("| 600519 | unknown | - |", response.text)
    ...
    def test_build_portfolio_section_renders_deterministic_overlay(self):
        command = AskCommand()
        results = {
            "600519": {
                "signal": "buy",
        ...
        self.assertIn("600519: 茅台 summary", text)
        self.assertIn("风险提示: 600519: 估值偏高", text)

    def test_build_portfolio_section_requires_two_results(self):
        command = AskCommand()
        results = {
            "600519": {
                "signal": "buy",
    ...
    def test_analyze_multi_uses_direct_bridge_text(self):
        command = AskCommand()
        config = SimpleNamespace()
        message = self._message()
        bridge = MagicMock()
        ...
        self.assertEqual(bridge.build_stock_answer.call_count, 2)

    def test_analyze_multi_persists_direct_bridge_history(self):
        command = AskCommand()
        config = SimpleNamespace()
        message = self._message()
        bridge = MagicMock()
        ...
        self.assertTrue(all("direct analysis" in text for text in assistant_messages))
    ...
    def test_analyze_single_uses_direct_bridge_with_requested_skill(self):
        command = AskCommand()
        config = SimpleNamespace()
        message = self._message()
        bridge = MagicMock()
