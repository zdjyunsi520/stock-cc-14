...
class SystemConfigServiceTestCase(unittest.TestCase):
    ...
    def test_get_setup_status_accepts_runtime_anthropic_key_with_opus_default(self) -> None:
        self._rewrite_env("")

        with patch.dict(
            os.environ,
            {
                "ANTHROPIC_API_KEY": "sk-ant-runtime-secret",
                "STOCK_LIST": "600519",
            },
            clear=True,
        ):
        ...
        self.assertTrue(status["is_complete"])
        self.assertEqual(checks["llm_primary"]["status"], "configured")
        self.assertIn("anthropic/claude-opus-4-6", checks["llm_primary"]["message"])
        self.assertEqual(checks["llm_agent"]["status"], "inherited")
