...
class SystemConfigService:
    ...
    def _infer_setup_legacy_primary_model(cls, effective_map: Dict[str, str]) -> str:
        ...
        if cls._has_any_config_value(effective_map, ("ANTHROPIC_API_KEYS", "ANTHROPIC_API_KEY")):
            model = (effective_map.get("ANTHROPIC_MODEL") or "claude-opus-4-6").strip()
    ...
    def _build_setup_notification_check(self, effective_map: Dict[str, str]) -> Dict[str, Any]:
        configured = (
            ...
            or (
                self._has_any_config_value(effective_map, ("FEISHU_APP_ID",))
                ...
                and (
                    ...
                    or self._has_any_config_value(effective_map, ("FEISHU_APP_CHAT_ID",))
                )
            )
        )
        if configured:
    ...
    def _validate_cross_field(effective_map: Dict[str, str], updated_keys: Set[str]) -> List[Dict[str, Any]]:
        ...
        has_feishu_app_chat_id = bool((effective_map.get("FEISHU_APP_CHAT_ID") or "").strip())
        ...
        if (
            ...
            and not (has_feishu_app_chat_id and has_feishu_app_id and has_feishu_app_secret)
