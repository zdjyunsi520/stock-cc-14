...
class Config:
    ...
    feishu_app_chat_id: Optional[str] = None
    ...
    anthropic_model: str = "claude-opus-4-6"  # Claude model name
    ...
    # === 实时盯盘配置（只读提醒，不执行交易）===
    realtime_watch_enabled: bool = False
    realtime_watch_symbols: List[str] = field(default_factory=list)
    realtime_watch_market_enabled: bool = True
    realtime_watch_hotspots_enabled: bool = True
    realtime_watch_market_region: str = ""
    realtime_watch_interval_seconds: int = 60
    realtime_watch_trading_hours_only: bool = True
    realtime_watch_notification_cooldown_seconds: int = 300
    realtime_watch_quote_cache_ttl_seconds: int = 30
    realtime_watch_market_cache_ttl_seconds: int = 60
    realtime_watch_hotspot_cache_ttl_seconds: int = 120
    realtime_watch_price_change_threshold_pct: float = 2.0
    realtime_watch_volume_ratio_threshold: float = 2.0
    realtime_watch_market_change_threshold_pct: float = 0.8
    realtime_watch_hotspot_change_threshold_pct: float = 2.0
    realtime_watch_hotspot_top_n: int = 5

    # === 盘中选股自动推送配置（只读提醒，不执行交易）===
    intraday_pick_enabled: bool = False
    intraday_pick_use_claude: bool = True
    intraday_pick_dry_run: bool = True
    intraday_pick_push_times: List[str] = field(default_factory=lambda: ["10:20", "13:45", "14:35"])
    intraday_pick_interval_seconds: int = 60
    intraday_pick_time_window_minutes: int = 3
    intraday_pick_trading_hours_only: bool = True
    intraday_pick_require_intraday_pattern: bool = True
    intraday_pick_max_candidates_for_deep_check: int = 80
    intraday_pick_report_limit: int = 10
    intraday_pick_claude_timeout_seconds: float = 60.0
    intraday_pick_claude_max_tokens: int = 1200
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        realtime_watch_symbols = [
            (c or "").strip().upper()
            for c in os.getenv('REALTIME_WATCH_SYMBOLS', '').split(',')
            if (c or "").strip()
        ]
        ...
        if not litellm_model:
            ...
            _anthropic_model_name = os.getenv('ANTHROPIC_MODEL', 'claude-opus-4-6').strip()
            if gemini_api_keys:
        ...
        return cls(
            ...
            feishu_app_chat_id=os.getenv('FEISHU_APP_CHAT_ID'),
            ...
            realtime_watch_enabled=parse_env_bool(os.getenv('REALTIME_WATCH_ENABLED'), default=False),
            realtime_watch_symbols=realtime_watch_symbols,
            realtime_watch_market_enabled=parse_env_bool(os.getenv('REALTIME_WATCH_MARKET_ENABLED'), default=True),
            realtime_watch_hotspots_enabled=parse_env_bool(os.getenv('REALTIME_WATCH_HOTSPOTS_ENABLED'), default=True),
            realtime_watch_market_region=os.getenv('REALTIME_WATCH_MARKET_REGION', '').strip().lower(),
            realtime_watch_interval_seconds=parse_env_int(
                os.getenv('REALTIME_WATCH_INTERVAL_SECONDS'),
                60,
                field_name='REALTIME_WATCH_INTERVAL_SECONDS',
                minimum=30,
            ),
            realtime_watch_trading_hours_only=parse_env_bool(
                os.getenv('REALTIME_WATCH_TRADING_HOURS_ONLY'),
                default=True,
            ),
            realtime_watch_notification_cooldown_seconds=parse_env_int(
                os.getenv('REALTIME_WATCH_NOTIFICATION_COOLDOWN_SECONDS'),
                300,
                field_name='REALTIME_WATCH_NOTIFICATION_COOLDOWN_SECONDS',
                minimum=0,
            ),
            realtime_watch_quote_cache_ttl_seconds=parse_env_int(
                os.getenv('REALTIME_WATCH_QUOTE_CACHE_TTL_SECONDS'),
                30,
                field_name='REALTIME_WATCH_QUOTE_CACHE_TTL_SECONDS',
                minimum=0,
            ),
            realtime_watch_market_cache_ttl_seconds=parse_env_int(
                os.getenv('REALTIME_WATCH_MARKET_CACHE_TTL_SECONDS'),
                60,
                field_name='REALTIME_WATCH_MARKET_CACHE_TTL_SECONDS',
                minimum=0,
            ),
            realtime_watch_hotspot_cache_ttl_seconds=parse_env_int(
                os.getenv('REALTIME_WATCH_HOTSPOT_CACHE_TTL_SECONDS'),
                120,
                field_name='REALTIME_WATCH_HOTSPOT_CACHE_TTL_SECONDS',
                minimum=0,
            ),
            realtime_watch_price_change_threshold_pct=parse_env_float(
                os.getenv('REALTIME_WATCH_PRICE_CHANGE_THRESHOLD_PCT'),
                2.0,
                field_name='REALTIME_WATCH_PRICE_CHANGE_THRESHOLD_PCT',
                minimum=0.0,
            ),
            realtime_watch_volume_ratio_threshold=parse_env_float(
                os.getenv('REALTIME_WATCH_VOLUME_RATIO_THRESHOLD'),
                2.0,
                field_name='REALTIME_WATCH_VOLUME_RATIO_THRESHOLD',
                minimum=0.0,
            ),
            realtime_watch_market_change_threshold_pct=parse_env_float(
                os.getenv('REALTIME_WATCH_MARKET_CHANGE_THRESHOLD_PCT'),
                0.8,
                field_name='REALTIME_WATCH_MARKET_CHANGE_THRESHOLD_PCT',
                minimum=0.0,
            ),
            realtime_watch_hotspot_change_threshold_pct=parse_env_float(
                os.getenv('REALTIME_WATCH_HOTSPOT_CHANGE_THRESHOLD_PCT'),
                2.0,
                field_name='REALTIME_WATCH_HOTSPOT_CHANGE_THRESHOLD_PCT',
                minimum=0.0,
            ),
            realtime_watch_hotspot_top_n=parse_env_int(
                os.getenv('REALTIME_WATCH_HOTSPOT_TOP_N'),
                5,
                field_name='REALTIME_WATCH_HOTSPOT_TOP_N',
                minimum=1,
                maximum=20,
            ),
            intraday_pick_enabled=parse_env_bool(os.getenv('INTRADAY_PICK_ENABLED'), default=False),
            intraday_pick_use_claude=parse_env_bool(os.getenv('INTRADAY_PICK_USE_CLAUDE'), default=True),
            intraday_pick_dry_run=parse_env_bool(os.getenv('INTRADAY_PICK_DRY_RUN'), default=True),
            intraday_pick_push_times=[
                item.strip()
                for item in os.getenv('INTRADAY_PICK_PUSH_TIMES', '10:20,13:45,14:35').split(',')
                if item.strip()
            ],
            intraday_pick_interval_seconds=parse_env_int(
                os.getenv('INTRADAY_PICK_INTERVAL_SECONDS'),
                60,
                field_name='INTRADAY_PICK_INTERVAL_SECONDS',
                minimum=30,
            ),
            intraday_pick_time_window_minutes=parse_env_int(
                os.getenv('INTRADAY_PICK_TIME_WINDOW_MINUTES'),
                3,
                field_name='INTRADAY_PICK_TIME_WINDOW_MINUTES',
                minimum=0,
            ),
            intraday_pick_trading_hours_only=parse_env_bool(
                os.getenv('INTRADAY_PICK_TRADING_HOURS_ONLY'),
                default=True,
            ),
            intraday_pick_require_intraday_pattern=parse_env_bool(
                os.getenv('INTRADAY_PICK_REQUIRE_INTRADAY_PATTERN'),
                default=True,
            ),
            intraday_pick_max_candidates_for_deep_check=parse_env_int(
                os.getenv('INTRADAY_PICK_MAX_CANDIDATES_FOR_DEEP_CHECK'),
                80,
                field_name='INTRADAY_PICK_MAX_CANDIDATES_FOR_DEEP_CHECK',
                minimum=1,
            ),
            intraday_pick_report_limit=parse_env_int(
                os.getenv('INTRADAY_PICK_REPORT_LIMIT'),
                10,
                field_name='INTRADAY_PICK_REPORT_LIMIT',
                minimum=1,
                maximum=20,
            ),
            intraday_pick_claude_timeout_seconds=parse_env_float(
                os.getenv('INTRADAY_PICK_CLAUDE_TIMEOUT_SECONDS'),
                60.0,
                field_name='INTRADAY_PICK_CLAUDE_TIMEOUT_SECONDS',
                minimum=1.0,
            ),
            intraday_pick_claude_max_tokens=parse_env_int(
                os.getenv('INTRADAY_PICK_CLAUDE_MAX_TOKENS'),
                1200,
                field_name='INTRADAY_PICK_CLAUDE_MAX_TOKENS',
                minimum=256,
            ),
            wechat_webhook_url=os.getenv('WECHAT_WEBHOOK_URL') or os.getenv('WECOM_BOT_WEBHOOK'),
    ...
    def _parse_llm_channels(cls, channels_str: str) -> List[Dict[str, Any]]:
        ...
        Format:
            ...
            LLM_AIHUBMIX_MODELS=gpt-5.5,claude-opus-4-6
