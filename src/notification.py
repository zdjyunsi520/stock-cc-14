...
class NotificationService(
    ...
    def detect_configured_channels(config: Config) -> List[NotificationChannel]:
        ...
        if (
            ...
            or (
                ...
                and getattr(config, "feishu_app_chat_id", None)
            )
        ):
    ...
    def _send_feishu_static(self, content: str) -> bool:
        """通过飞书静态渠道发送：优先 Webhook，否则使用应用机器人目标群。"""
        if getattr(self._config, "feishu_webhook_url", None):
            return self.send_to_feishu(content)

        chat_id = (getattr(self._config, "feishu_app_chat_id", None) or "").strip()
        if not chat_id:
            logger.warning("飞书应用目标群 FEISHU_APP_CHAT_ID 未配置，跳过应用机器人推送")
            return False
        return self._send_feishu_stream_reply(chat_id, content)
    ...
    def _send_to_static_channel(
        ...
        *,
        ...
        if channel == NotificationChannel.FEISHU:
            return self._send_feishu_static(content)
